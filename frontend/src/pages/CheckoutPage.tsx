...
export function CheckoutPage() {
  ...
  if (cartItems.length === 0) {
    return (
      <div className="mx-auto max-w-lg px-4 py-20 text-center">
        ...
        <button onClick={() => navigate('/products')} className="rounded bg-tealbrand px-6 py-2 font-semibold text-white">
          Shop now
        </button>
      </div>
    );
  ...
  async function handleCheckout(e: FormEvent) {
    ...
    try {
      ...
      // Redirect to Stripe or mock success
  ...
  return (
    <div className="mx-auto max-w-5xl gap-8 px-4 py-10 lg:grid lg:grid-cols-[1fr_380px]">
      <div>
        ...
        <form onSubmit={handleCheckout} className="grid gap-6">
          ...
          <button
            type="submit"
            disabled={loading}
            className="h-12 w-full rounded bg-tealbrand text-base font-semibold text-white disabled:bg-slate-300"
          >
      ...
      <aside className="hidden lg:block">
        <div className="sticky top-24 rounded-lg border border-slate-200 p-6">
